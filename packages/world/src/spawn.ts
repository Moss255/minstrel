...
export function findSpawn(world: CollisionWorld, options: SpawnOptions): Spawn | undefined {
  ...
  // **Rounded, because a scaled map's bounds are not whole words.** A mesh
  // scaled into the world need not land on whole `fx32` words, and the village
  // inn's `maxY` once came out at 5235.5 — which `fx32` refuses, so looking for
