...
| **How the chance meets a draw** | `func_ov000_02156cc4`, the one caller: `NextRandomMax(10000) < (int)(100.0f × rate)` — the multiply a float's, the `int` a truncation | `below(10_000) < critical` | **the same shape, settled.** Exact to deftness 150, which is all the slice reaches. **Past 150 ours is one too high at 151 of the 850 values**, because `0.01f` is not a hundredth and the game truncates. The float question again, and far commoner — see below |
...
It shows in two places, and the second is much the larger:

- **a draw below 10,000 differs from the game's about once in four thousand**,
  by one. The draw *count* never differs, so the generator never drifts;
- **the critical threshold is one too high at 151 of the 850 deftness values
  past 150** — about one in six. Nothing in the slice has deftness past 150,
  so it bites nobody yet, and it will bite the rest of the game.
