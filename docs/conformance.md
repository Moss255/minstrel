...
| **Initiative** | inlined in `ProcessCombatTurn`: the **buffed** agility, capped at 999, times `NextRandomFloatBetween(0.51, 1.0)`, sorted highest first by a quicksort over floats | `initiative`, over `levelled(agility)` | **the game's arithmetic**, and the agility is the buffed one. `0.51f` appears once in the whole build. **Ours**: how ties break, the game's sort being unstable |
